...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？
    答:n个节点经过n-1次合并最终形成哈夫曼树，每一次合并会产生一个新节点，经过n-1次合并生成n-1个新节点，因此节点总数变成2n-1。
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？
    答：(1).读小写字母，将调用ASCII字符表时遍历26个大写字母（0X41）改为遍历26个小写字母（0x61）；（2）.同时对大小写字母编码，就可以改为遍历ASCII字符表中从“A”到“z”的所有字符，（0x41）不用改，将数组arr_big[]的范围扩大，改为arr_big[57]，然后将遍历时j的范围改为j<57。
