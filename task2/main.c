...
int main()
{
    ...
    char str[100]={'D','E','A','E','D','D','E','D','B','C','E','E','E','C','E','C','D','E','E','D','D','B','E','E','E','C','E','E','D','E','E'};
    ...
    for(int i = 0; i < 27; i++)
    {
        if(arr_big[i] != 0)
        {
            arr_small[count2++] = arr_big[i];	/*将arr_big[]中的值赋给arr_small[],去除arr_big[]中的多余数据*/
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  /*如果当前节点分别是左右叶节点*/
        {
            length = findParent(i,huffman,count);   //得到各个叶结点的带权路径长度
        	
            sum = sum + length*arr_small[i];	//各个叶结点带权路径长度累加得到总长度WPL
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /* 如果该点的权重为0，并且该点没有父节点*/
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  /* 如果该点的权重小于min，并且该点没有父节点，将该点权重赋值给min*/
        {
            min = huffman[i][3];
            ii[0] = i;
        }
